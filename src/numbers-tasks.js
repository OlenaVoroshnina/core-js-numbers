...
function roundToPowerOfTen(num, pow) {
  // Calculate the divisor by raising 10 to the power of the specified exponent
  const divisor = 10 ** pow;

  // Divide the number by the divisor, round the result, and multiply by the divisor
...
function isPrime(n) {
  ...
  for (let i = 2; i <= Math.sqrt(n); i += 1) {
    // If the number is divisible evenly, it's not prime
    if (n % i === 0) {
      return false;
    }
  }
...
function toNumber(value, def) {
  ...
  // Check if the conversion was successful using Number.isNaN
...
function getSumOfDigits(num) {
  ...
  // Sum the digits by iterating through the string
  const sum = numStr
    .split('')
...
function getRandomInteger(min, max) {
  ...
  const randomInteger = Math.floor(randomDecimal * (maxN - minN + 1)) + minN;
